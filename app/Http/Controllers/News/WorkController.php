...
class WorkController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $list=News::where('n_type',5)->wherein('status',[1,3,4])->orderBy('created_at','desc');
//        if($status){
...
//        }
        ...
        $url = $request->fullUrl();
        return view('news.work.index',compact('list','url','status','users','medias','starttime','endtime','search'));
    }
    ...
    public function addSave(Request $request){
        $m=new News();
        $m->n_type=5;
        $m->created_at=$request->input('created_at');
...
//        $m->at_id=Auth::user()->id;
    ...
    public function look($id)
    {
        ...
        $users=Users::pluck('name','id');
        $medias=Media::pluck('media_name','id');
        return view('news.work.info',compact('url','info','medias','users'));
    }
    ...
    //导出 記者上稿
    public function export_csv($list,$export_name='工作台帐'){
